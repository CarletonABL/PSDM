function [Q, Qd, Qdd, tau] = genTestPoses(DH_ext, X, g_in, Nq, Nt, type_in)
    ...
    if strcmp(type, 'gravity')
        ...
        if nargout > 3
            ...
            for j = 1:Nt
               tau(:, :, j) = PSDM.inverseDynamicsNewton(DH_ext, Xlist(:, :, j), Q, Qd, Qdd, int8(2), g)';
            end
    ...
    elseif strcmp(type, 'accel')
        ...
        if nargout > 3
            ...
            tau = zeros(Nq, DOF, Nt);
            for j = 1:Nt
               tau(:, :, j) = PSDM.inverseDynamicsNewton(DH_ext, Xlist(:, :, j), Q, Qd, Qdd, int8(2), [0 0 0]')';
            end
    ...
    elseif strcmp(type, 'velocity') || ...
        ...
        if nargout > 3
            ...
            for j = 1:Nt
               ...
               tau_j = PSDM.inverseDynamicsNewton(DH_ext(:, :), Xlist(:, :, j), Q, Qd, Qdd, int8(2), [0 0 0]')';
